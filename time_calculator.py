...
def end_days_later_text(end_days_later, start_day, weekdays):
    ...
    if end_days_later == 1 and start_day != "":
        ...
        start_index = weekdays.index(start_day_normalize)
        updated_day = ""
    ...
    if end_days_later > 1 and start_day != "":
        start_index = weekdays.index(start_day_normalize)
        updated_day = ""
        days_to_move_forward = end_days_later % 7  # the reminder will be the nr of days to move
        update_week = weekdays[start_index:] + weekdays[:start_index]
        updated_day += update_week[update_week.index(start_day_normalize) + days_to_move_forward]
        return ", " + updated_day + " (" + str(end_days_later) + " days later)"
    if end_days_later > 1 and start_day == "":
